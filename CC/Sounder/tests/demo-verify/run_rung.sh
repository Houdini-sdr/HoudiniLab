...
setsid nohup timeout -s INT $(( $3 + 15 )) python3 csi_gui/csi_server.py > ap79_runs/$1_csi_$T.log 2>&1 < /dev/null &
sleep 2
setsid nohup timeout -s INT $3 ./build/sounder --view --conf_file $2 > ap79_runs/$1_$T.log 2>&1 < /dev/null &
sleep 1
SP=$(pgrep -x sounder | head -1)
...
echo "launched $1_$T at $(date -u +%H:%M:%SZ) (sounder pid $SP)"
