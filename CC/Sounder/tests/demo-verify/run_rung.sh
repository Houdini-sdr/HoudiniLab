...
T=$(date +%H%M%S); echo "$1_$T" > "ap79_runs/$1.current" || exit 1  # before anything is launched
setsid nohup timeout -s INT $(( $3 + 15 )) python3 csi_gui/csi_server.py --conf "$2" > "ap79_runs/$1_csi_$T.log" 2>&1 < /dev/null &
echo $! > "ap79_runs/$1.csipid"  # the dashboard's timeout wrapper: fstage_run stops it when the sounder is done
sleep 2
