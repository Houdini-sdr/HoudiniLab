...
const std::vector<SyncConfig::Spec>& SyncConfig::schema() {
  static const std::vector<Spec> kSchema = {
      ...
      {"detector.first_path_guard", "HOUDINI_FIRST_PATH_GUARD", 0, 4,
       "Samples immediately before the peak the first-path search skips. A beacon between samples splits its peak over two adjacent taps and the earlier one is the SAME arrival, not an earlier one; 1 skips it. 0, the default, is what every release so far has shipped.",
