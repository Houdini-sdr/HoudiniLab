...
| 4.38 | RESIDUAL QUALITY VARIANCE after the ring fix, current state: with the two-stage timing recovery (integer blind search +-8 + deterministic pilot-tone fraction, e8e456c) and the HONEST phase-only metric, 12 restarts span 0.23-0.998 with one ring-band run -- and the killer pair persists (0.998 vs 0.231 at pgo 212/213). Single-frame dumps of low-score runs demodulate at 0.90+ coherence, so the damage is FRAME-TO-FRAME in the live path. Leading suspects: per-frame integer-stage flapping or per-frame U-slot extraction excursions beyond the CP margin (ISI class). HOUDINI_CSI_R_DEBUG logs per-frame r (landed in e8e456c) but the campaign's detached launch failed to deliver the env -- next session: one low-score run with the debug confirmed, then median-filter r over frames OR timestamp-derived extraction (the root-cause reduction). NOTE metric thresholds changed (phase-only): scores are not comparable to pre-4dbd60c campaigns; the STATIC-ramp ring mechanism (4.36) remains fixed-and-verified | ap15_final2 campaign; offline diffs of runs 8/11/9 | VERIFIED-HW (distribution); residual mechanism OPEN |
