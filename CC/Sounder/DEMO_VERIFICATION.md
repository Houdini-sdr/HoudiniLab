...
| # | Claim | Evidence | Status |
|---|---|---|---|
| 4.16 | CORRECTED (the initial "bench not in demo cabling" reading was WRONG -- [user] supplied the intent, measurement then confirmed). Measured cable map this boot: SELF-loopback TX0->RX0 on BOTH boards (.21 rms 187 / .22 rms 289; levels differ 4.5 dB, consistent with two physical cables); the TWO-NODE cable is .22 TX1 -> .21 RX1 (rms 284, ratio 5.6e9) -- channel "B" both ends, matching the config's B/B, but the DIRECTION is reverse of topology-houdini.json roles (BS0=.21). All other 14 combos = crosstalk floor (absmax ~20). DECISION NEEDED [user]: swap topology roles (BS=.22, UE=.21) or reverse the cable | map_scan.py all four orientations + liveness counters | VERIFIED-HW |
