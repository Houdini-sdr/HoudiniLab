...
| `sync.detector.first_path_guard` | 0 | `HOUDINI_FIRST_PATH_GUARD` | 0 to 4 | clamped | Samples immediately before the peak the first-path search skips. A beacon between samples splits its peak over two adjacent taps and the earlier one is the SAME arrival, not an earlier one; 1 skips it. 0, the default, is what every release so far has shipped. |
