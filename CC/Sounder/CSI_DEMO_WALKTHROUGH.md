...
| `HOUDINI_TX_AFFINITY`, `HOUDINI_RX_AFFINITY` | unset (the kernel places them) | The core for the SDR plugin's own worker threads: the client's live transmit pacer, and every receive loop. A comma list for `HOUDINI_TX_AFFINITY` gives each transmit stream its own core, in channel order. `HOUDINI_TX_RTPRIO` (1 to 99) runs the transmit pacers at that real-time priority (the sounder needs the CAP_SYS_NICE capability for it; without it the plugin warns and runs at normal priority). Two syntaxes: `HOUDINI_TX_AFFINITY` is split into ONE core per transmit stream, while `HOUDINI_RX_AFFINITY` is passed through as the plugin's own CPU set for its receive loop. Neither is shifted by `HOUDINI_CORE_BASE`, so pick cores clear of the ones the sounder pins. Keep them off the core the sounder's dispatch thread spins on (`HOUDINI_CORE_BASE`, default 0): a pacer that shares it sends late. |
...
| `sync.steer.enable` | false | `HOUDINI_CLOCK_STEER` |  | refused | Steer the UE's clock onto the beacon's with CLOCK_ADJ, from the tracked grid rate, inside the sounder. Needs the UE's clock_ref to be calibrated. Off by default. |
...
| `sync.steer.max_push` | 2 |  | 1 to 50 |  | Most counts one push may move, so no single frequency step is large. |
