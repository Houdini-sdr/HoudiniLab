...
| `HOUDINI_BEACON_PICK` | unset | Diagnostic. Unset means the targeted resync search returns the STRONGEST correlation crossing in its slice, which is the shipped behaviour since 2026-09-02. Set it to `first` to restore the older rule, which returned the EARLIEST crossing. The older rule is not safe: the beacon's own STS preamble repeats every 16 samples, 16 divides the 128-sample correlator lag, and so the preamble looks to the detector exactly like the repeated field it is hunting for. On a strong enough link the search locks onto the preamble a few hundred samples early. Whether it does depends on received level, so a link that is fine today can fail after a gain change. This knob exists so both rules can be run on one build for comparison; leave it unset. |
