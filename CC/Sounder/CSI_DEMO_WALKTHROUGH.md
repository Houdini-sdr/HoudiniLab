...
| `HOUDINI_TX_AFFINITY`, `HOUDINI_RX_AFFINITY` | unset (the kernel places them) | The core for the SDR plugin's own worker threads: the client's live transmit pacer, and every receive loop. A comma list for `HOUDINI_TX_AFFINITY` gives each transmit stream its own core, in channel order. `HOUDINI_TX_RTPRIO` (1 to 99) runs the transmit pacers at that real-time priority. Keep them off the core the sounder's dispatch thread spins on (`HOUDINI_CORE_BASE`, default 0): a pacer that shares it sends late. |
