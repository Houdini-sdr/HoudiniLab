...
| `HOUDINI_TX_AFFINITY`, `HOUDINI_RX_AFFINITY` | unset (the kernel places them) | The core for the SDR plugin's own worker threads: the client's live transmit pacer, and every receive loop. Keep them off the core the sounder's dispatch thread spins on (`HOUDINI_CORE_BASE`, default 0): a pacer that shares it sends late. |
