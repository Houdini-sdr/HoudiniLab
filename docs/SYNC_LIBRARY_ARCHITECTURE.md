...
- Startup prints the effective struct with one of `default`, `json`, `env`
  beside every value. With `allow_env_overrides: false` (the default) an
  `HOUDINI_*` knob that would have changed a value is reported and ignored.
  The recommendation is to keep the override path for one release and then
  remove it.
